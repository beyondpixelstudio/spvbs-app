...
export default async function CentralCommitteePage() {
    ...
    <div className="min-h-screen bg-[#f7f4ee]">
      <div className="bg-[var(--color-bg-secondary)] py-[50px] px-[20px]">
        <div className="max-w-[1200px] mx-auto">
          <h1 className="!text-[42px] !text-white font-[family-name:var(--font-heading)]">Central Committee</h1>
          <p className="text-[17px] text-[#cedbf5] mt-[8px]">
            The dedicated members serving our samaj at the central level.
          </p>
        </div>
      </div>

      <div className="max-w-[1200px] mx-auto px-[20px] py-[40px]">
        <CommitteeDisplay members={members} typeOptions={Object.entries(typeLabel).map(([value, label]) => ({ value, label }))} level="CENTRAL" />
      </div>
    </div>
  );
}
